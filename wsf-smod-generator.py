...
def main():
    ...
    parser.add_argument(
        "-cf", "--confidenceFile", help="Full file path of GHS-Built(Confidence) raster TIF file ")

    parser.add_argument(
        "-pr", "--predictionFile", help="Full file path of GHS-Built(Prediction) raster TIF file ")
    parser.add_argument("-p", "--popFile",
                        help="Full file path of GHS-POP raster TIF file ")
    ...
    if(built_file is None or pop_file is None or prediction_file is None):
        print("GHS-BUILT (confidence) or prediction or GHS-POP file is missing")
        exit()
    ...
    for ix, iy in tqdm(np.ndindex(final_ghs_pop2d.shape)):
        ...
        if((popcellvalue > 1000 or confidencecellvalue > 80) and (np.sum(classify_urban_area(ix, iy, 10, final_ghs_pop2d)) > 50000) and predictioncellvalue == 0):
            urban_array[ix, iy] = 3
        elif (popcellvalue > 100 and (np.sum(classify_urban_area(ix, iy, 8, final_ghs_pop2d)) > 5000) and predictioncellvalue == 0):
            urban_array[ix, iy] = 2
        elif(popcellvalue > 1 and (np.sum(classify_urban_area(ix, iy, 8, final_ghs_pop2d)) < 5000) and predictioncellvalue == 0):
            urban_array[ix, iy] = 1
