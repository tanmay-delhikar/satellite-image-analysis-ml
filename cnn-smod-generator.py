...
def main():
    ...
    with rasterio.open(built_file.strip()) as src_built:
        ...
        if(x_size_built != y_size_built):
            print('Please input n x n BUILT tif files only')
            exit()
    ...
    for ix, iy in tqdm(np.ndindex(final_ghs_pop2d.shape)):
        ...
        if((popcellvalue > 1000 or confidencecellvalue > 80) and (np.sum(classify_urban_area(ix, iy, 10, final_ghs_pop2d)) > 50000) and predictioncellvalue == 1):
            urban_array[ix, iy] = 3
        elif (popcellvalue > 100 and (np.sum(classify_urban_area(ix, iy, 8, final_ghs_pop2d)) > 5000) and predictioncellvalue == 1):
            urban_array[ix, iy] = 2
        elif(popcellvalue > 1 and (np.sum(classify_urban_area(ix, iy, 8, final_ghs_pop2d)) < 5000) and predictioncellvalue == 1):
            urban_array[ix, iy] = 1
