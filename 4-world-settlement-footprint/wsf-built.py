...
def main():
    ...
    create_new_tif(temporal_files[0], str(os.path.join(data_directory, '30m-built-confidence.tif')),
                   np.asarray(class_0_probs, dtype='float32').reshape(IMG_SIZE, IMG_SIZE), 'float32', 1)
    change_pixel_size(os.path.join(data_directory, '30m-built-prediction.tif'),
                      os.path.join(data_directory, '10m-built-prediction.tif'), 10)
    change_pixel_size(os.path.join(data_directory, '30m-built-confidence.tif'),
                      os.path.join(data_directory, '10m-built-confidence.tif'), 10)
    ...
    clip_from_vrt(os.path.join(data_directory, '10m-built-prediction.tif'), vrt_file, os.path.join(
        data_directory, 'trash2.tif'), os.path.join(data_directory, '10m-original-built.tif'))
    ...
    dictionary = classification_report(Y_list, Y_pred, output_dict=True)
    print(dictionary)
    intersection = np.logical_and(Y_list, Y_pred)
    union = np.logical_or(Y_list, Y_pred)
