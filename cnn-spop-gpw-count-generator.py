...
def main():
    parser = argparse.ArgumentParser(
        description="CREATES POP from BUILT (confidence) and GPW data")
    parser.add_argument("-c", "--config", help="Full file path of config.json")
    parser.add_argument(
        "-cf", "--inputFile", help="Full file path of Built(Confidence) raster TIF file ")
    parser.add_argument(
    ...
    if(input_file is None or prediction_file is None):
        print("BUILT (confidence) or (prediction) input TIF file is missing")
        exit()
    ...
    # POP GENERATION WITH DASYMMETRIC ALGORITHM, REFER TO THE PAPER FOR DESCRIPTION
    ...
    confidence_values_category = 100*confidence_values_category

    a = None
    confidence_values_category[confidence_values_category < 50] = 0
    confidence_values_category[(confidence_values_category < 55) & (
        confidence_values_category >= 50)] = 1
    confidence_values_category[(confidence_values_category < 60) & (
        confidence_values_category >= 55)] = 2
    confidence_values_category[(confidence_values_category < 65) & (
        confidence_values_category >= 60)] = 3
    confidence_values_category[(confidence_values_category < 70) & (
        confidence_values_category >= 65)] = 4
    confidence_values_category[(confidence_values_category < 80) & (
        confidence_values_category >= 70)] = 5
    confidence_values_category[confidence_values_category >= 80] = 6
