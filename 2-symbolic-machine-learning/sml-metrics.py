...
def main():
    parser = argparse.ArgumentParser(
        description="Calculates metrics for SML-based BUILT and Original GHSL-BUILT")
    parser.add_argument("-pr", "--prediction", help="Full file path of SML based BUILT prediction")
    parser.add_argument("-og", "--original", help="Full file path of original GHSL BUILT")
    parser.add_argument("-of", "--outputFolder", help="Full desired output folder path")
    ...
    if (prediction_file is None or prediction_file.isspace()):
        print('SML Based BUILT prediction file is missing')
        exit()
    ...
    if(x_size_pred!=x_size_org):
        print('SML BUILT file and Original GHSL BUILT file sizes do not match')
        exit()
    ...
    built_org[built_org==255]=2

    built_sml1=gdal.Open(prediction_file)
    built_sml = built_sml1.GetRasterBand(1).ReadAsArray()
    dictionary=classification_report(built_org.flatten(),built_sml.flatten(),output_dict=True)
